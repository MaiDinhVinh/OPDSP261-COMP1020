...
public class Lab7_P1_V202502959 {
    public static void main(String[] args) {
        try(Scanner sc = new Scanner(System.in)){
            int count = sc.nextInt();
            sc.nextLine();
            // edge case: empty tree -> print 4 zeros as specified
            if(count == 0){
                ...
                BST bst = new Lab7_P1_V202502959().new BST();
                for(int i: arr) bst.add(i);

                // output order: root -> leaf count -> height -> inorder
                System.out.println(bst.getRoot().getValue());
                System.out.println(bst.getLeafNodeCount());
                System.out.println(bst.treeHeight());
                ArrayList<Integer> elements = bst.inOrderTraversal();

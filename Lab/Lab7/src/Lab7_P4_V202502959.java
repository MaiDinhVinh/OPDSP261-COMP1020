...
public class Lab7_P4_V202502959 {
    public static void main(String[] args) {
        try(Scanner sc = new Scanner(System.in)){
            ...
            if(count == 0){
                ...
                BST bst = new Lab7_P4_V202502959().new BST();
                for(int i: arr) bst.add(i);

                // balance check: diff > 1 means NOT BALANCED (AVL-style criterion)
                if(bst.checkBalance() > 1){
    ...
    class BST{
        ...
        // checks balance at the root only — compares height of left vs right subtree
        // returns the absolute difference; caller checks if > 1
        public int checkBalance(){
            return Math.abs(treeHeight(this.root.getLeft()) - treeHeight(this.root.getRight()));
        }
    }

...
public class Lab9_P4_V202502959 {
    ...
    public static void main(String[] args) {
        try (Scanner sc = new Scanner(System.in)) { // try-with-resources so scanner closes automatically
            ...
            Queue<Integer> queue = new LinkedList<>(); // BFS uses a queue — FIFO order
            int start = sc.nextInt();
            visited.add(start);
            queue.add(start);

            while(!queue.isEmpty()) {
                Vertex v = g.getVertex(queue.poll()); // dequeue and process
                for(int i: v.getNeighbors()) {
                    if(!visited.contains(i)) { // only enqueue unvisited neighbors
                        visited.add(i);
                        queue.add(i);
                    }
                }
                bfs.add(v.getValue()); // record visit order after processing each vertex
            }

...
public class Lab9_P5_V202502959 {
    static class Vertex {
        private int value;
    ...
    public static void main(String[] args) {
        try (Scanner sc = new Scanner(System.in)) { // try-with-resources so scanner closes automatically
            ...
            Stack<Integer> s = new Stack<>(); // DFS uses a stack — go deep before backtracking
            ...
            while(!s.isEmpty()) {
                Vertex v = g.getVertex(s.peek()); // peek to keep current vertex on stack until fully explored
                Vertex notVisited = null;
                for(int i: v.getNeighbors()) {
                    if(!visited.contains(i)) {
                        notVisited = g.getVertex(i); // grab the first unvisited neighbor
                        break;
                    }
                }
                if(notVisited != null) {
                    ...
                    s.push(notVisited.getValue()); // go deeper
                } else {
                    s.pop(); // no unvisited neighbors left, backtrack
                }
            }
            // Stream: convert each integer in the result list to string, then join with spaces for output
